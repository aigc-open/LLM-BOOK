...
### 2. [NPU适配实践指南](./NPU适配实践指南.md) 🔧 实战指南 ⭐ 推荐

**适合人群**：需要将 Liger-Kernel 适配到 NPU 的开发者（**适用于华为昇腾、寒武纪、燧原、壁仞等已支持 Triton 的 NPU**）

**内容概览**：
- ✅ 如何为 NPU 创建特化 Triton 算子（优化内存布局、块大小等）
- ✅ 保持客户使用方式完全不变（零代码修改）
- ✅ 设备自动检测和切换（NPU/CUDA/AMD 自动适配）
- ✅ 完整的代码实现示例（从检测到部署）
- ✅ 测试和性能分析方法（正确性验证、性能对比）

**核心解决方案**：通过条件 Monkey Patch 和设备感知模块，让客户代码零修改的同时享受 NPU 优化。

**预计工作量**：2-4 周（远少于完全重写的 3-6 个月）
...
#### 为 NPU 适配 Liger-Kernel（推荐方案）⭐ 
👉 阅读：[NPU适配实践指南](./NPU适配实践指南.md)  

**适用场景**：NPU 已支持 Triton（华为昇腾、寒武纪、燧原、壁仞等）

**核心方案**：
- ✅ 创建 NPU 特化算子（针对 NPU 优化 Triton 内核）
- ✅ 设备感知的 Module 自动切换（透明选择最优实现）
- ✅ **客户代码完全不变，零修改！**
- ⏱️ 预计工作量：2-4 周（远少于完全重写）

**实现效果**：
```python
# 客户代码保持完全一致
apply_liger_kernel_to_llama()  # 自动检测并使用 NPU 优化
model = AutoModelForCausalLM.from_pretrained("...")
# NPU 上自动使用特化内核，CUDA 上自动使用 CUDA 优化
```

#### 评估 NPU 迁移可行性（NPU 不支持 Triton - 较少见）
...
| **5. Monkey Patch 能否迁移到 NPU** | ✅ **可以！** 主流 NPU 已支持 Triton | **NPU 支持 Triton（推荐）**：[NPU适配指南](./NPU适配实践指南.md) ⭐ 2-4周<br>**NPU 不支持 Triton（罕见）**：[深度分析 - 第5章](./Liger-Kernel项目深度分析.md#5-npu-兼容性分析) 3-6月|
...
| 平台 | 状态 | 说明 |
|------|------|------|
| **NVIDIA GPU** | ✅ 完全支持 | 推荐使用，CUDA 原生支持 |
| **AMD GPU** | ✅ 完全支持 | ROCm 5.0+，官方支持 |
| **Intel GPU** | ⚠️ 实验性支持 | XPU，官方支持 |
| **NPU (支持 Triton)** | ✅ 可适配 | **华为昇腾、寒武纪、燧原等**已支持 Triton，见 [NPU适配指南](./NPU适配实践指南.md) ⭐ |
| **NPU (不支持 Triton)** | ⚠️ 需要完全重写 | 需要重新实现所有算子（3-6 个月），见[深度分析-第5章](./Liger-Kernel项目深度分析.md#5-npu-兼容性分析) |

> 💡 **好消息**：目前主流 NPU 厂商（华为昇腾、寒武纪、燧原、壁仞等）都在积极支持 Triton，可以通过 [NPU适配实践指南](./NPU适配实践指南.md) 快速适配，**客户代码零修改**！
...
- 🎯 提供保持客户代码不变的完整解决方案
- 💡 **适用于主流 NPU**：华为昇腾、寒武纪、燧原、壁仞等
- ⏱️ **快速适配**：预计 2-4 周即可完成（vs 完全重写需要 3-6 个月）
...
### 高级应用（NPU 适配）⭐
1. **[NPU适配实践指南](./NPU适配实践指南.md)** - NPU 适配（华为昇腾/寒武纪/燧原等）
2. [深度分析 - 第5章](./Liger-Kernel项目深度分析.md#5-npu-兼容性分析) - NPU 理论分析
3. [深度分析 - 第6章](./Liger-Kernel项目深度分析.md#6-单元测试使用方法) - 测试框架
4. 阅读源码：`src/liger_kernel/`
5. 贡献代码
